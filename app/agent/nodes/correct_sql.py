...
async def correct_sql(state: DataAgentState, runtime: Runtime[DataAgentContext] = None):
    ...
    try:
        ...
        error = state["error"]
        sql = state["sql"]
        prompt_template = PromptTemplate(template=load_prompt("correct_sql"),
                                         input_variables=["query", "metric_infos", "table_infos",
                                                          "date_info", "db_info", "error", "sql"])
        output_parser = StrOutputParser()
        chain = prompt_template | llm | output_parser
        res_sql = await chain.ainvoke({"query": query,
